...
async function processNode(executionId: string, conversationId: string, nodeId: string): Promise<void> {
...
async function handleDelayNode(ctx: ExecutionContext, executionId: string, conversationId: string, node: FlowNode) {
  ...
  // For MVP: use setTimeout (production should use BullMQ delayed jobs)
  await new Promise((resolve) => setTimeout(resolve, Math.min(data.delayMs ?? 0, 30000)))
  await goToNext(ctx, executionId, conversationId, node.id)
}

async function handleLiveChatNode(ctx: ExecutionContext, executionId: string, conversationId: string) {
  await prisma.conversation.update({
    where: { id: conversationId },
