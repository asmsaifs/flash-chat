(function () {
  ...
  panel.innerHTML = [
    '<div id="fc-head">',
    '  <div id="fc-av">FC</div>',
    '  <div><div id="fc-head-title">Support</div><div id="fc-head-sub">We reply in minutes</div></div>',
    '</div>',
    '<div id="fc-msgs"></div>',
    '<div id="fc-footer">',
  ...
  function poll() {
    ...
    if (state.lastSentAt) url += '?after=' + encodeURIComponent(state.lastSentAt);
    fetch(url, { headers: { Authorization: 'Bearer ' + state.visitorToken } })
      .then(function (r) { return r.ok ? r.json() : null; })
      .then(function (res) {
        if (!res || !res.data || !res.data.length) return;
        state.messages = state.messages.concat(res.data);
        state.lastSentAt = res.data[res.data.length - 1].sentAt;
        renderMessages();
      })
      .catch(function () {});
  }
  ...
  function sendMessage() {
    ...
    if (!state.conversationId) {
      initSession(function () { doSend(text); startPoll(); });
    } else {
      doSend(text);
    }
  }
